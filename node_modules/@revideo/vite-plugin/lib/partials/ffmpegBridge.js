...
exports.FFmpegBridge = exports.ffmpegBridgePlugin = void 0;
...
function ffmpegBridgePlugin({ output }) {
    return {
        ...
        configureServer(server) {
            ...
            server.middlewares.use('/audio-processing/generate-audio', async (req, res) => {
                if (req.method !== 'POST') {
                    return;
                }
                const { tempDir, assets, startFrame, endFrame, fps } = JSON.parse(await new Promise((resolve, reject) => {
                    let body = '';
                ...
                try {
                    await (0, ffmpeg_1.generateAudio)({
                        outputDir: output,
                        tempDir,
                        assets,
                        startFrame,
            ...
            server.middlewares.use('/audio-processing/merge-media', async (req, res) => {
                if (req.method !== 'POST') {
                    return;
                }
                const { outputFilename, tempDir } = JSON.parse(await new Promise((resolve, reject) => {
                    let body = '';
                    req.on('data', chunk => (body += chunk));
                    req.on('end', () => resolve(body));
                    req.on('error', reject);
                ...
                try {
                    await (0, ffmpeg_1.mergeMedia)(outputFilename, output, tempDir);
                    res.statusCode = 200;
                }
                catch (e) {
                    console.error(e);
...
class FFmpegBridge {
    constructor(ws, config) {
        ...
        this.handleDecodeVideoFrame = async ({ data }) => {
            ...
            if (!extractor) {
                extractor = new ffmpeg_1.VideoFrameExtractor(typedData.filePath, typedData.startTime, typedData.fps, typedData.duration);
